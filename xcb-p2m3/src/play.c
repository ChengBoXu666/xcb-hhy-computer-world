...
void play(Player *player, Card *card_fetch, Card *card_discard, Result result, Tran trans, FILE *file) // the main logical function of the game
{
    ...
    while (1)
    {
        ...
        fprintf(file, "Player %d's turn\n", player->index);
        printf("\n");
        fprintf(file, "\n");
        if (result.demo_mode == 0)
            sleep(1);
        printf("Player %d current cards: ", player->index);
        fprintf(file, "Player %d current cards: ", player->index);
        ...
        if (result.demo_mode == 0) // an extra judgement for the demo mode
        {
            ...
            fprintf(file, "Please input your choice(Input an integar: 0:no card to play/not play, 1:Card1, 2:Card2, 3:Card3, etc.): ");
            scanf("%d", &num);
            fprintf(file, "%d", num);
            printf("\n");
            fprintf(file, "\n");
        ...
        else
        {
            ...
            fprintf(file, "Player %d plays card: ", player->index);
        ...
        if ((table.attack != 0) && ((flag == 0 || num == 0))) // fetch the cards from the stock pile
        {
            ...
            fprintf(file, "Player %d draws %d cards: ", player->index, table.attack);
        ...
        fprintf(file, "Current card on table: ");
        card_output(table.current->suit, table.current->rank, file);
        printf("\n\n");
        fprintf(file, "\n\n");
