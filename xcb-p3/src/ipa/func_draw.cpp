...
void Group::rotate(Vec r_anchor,float angle){
    setrota_anchor(r_anchor); setrota_angle(angle); setangle(getangle()+angle);
    setAnchor(getrota_anchor() + ((getAnchor()-getrota_anchor())<<angle));
    for(it=sh.begin();it!=sh.end();++it) (*it)->rotate(getrota_anchor(),getrota_angle());
}
void Group::zoom(){
...
void triangle::rotate(Vec r_anchor,float angle){
    setrota_anchor(r_anchor); setrota_angle(angle); setangle(getangle()+angle);
    setAnchor(getrota_anchor() + ((getAnchor()-getrota_anchor())<<angle));
    p1= getrota_anchor() + ((p1-getrota_anchor())<<(getrota_angle())); p2= getrota_anchor() + ((p2-getrota_anchor())<<(getrota_angle()));
    p3= getrota_anchor() + ((p3-getrota_anchor())<<(getrota_angle()));
}
void rectangle::rotate(Vec r_anchor,float angle){
    setrota_anchor(r_anchor); setrota_angle(angle); setangle(getangle()+angle);
    setAnchor(getrota_anchor() + ((getAnchor()-getrota_anchor())<<angle));
    p1= getrota_anchor() + ((p1-getrota_anchor())<<(getrota_angle())); p2= getrota_anchor() + ((p2-getrota_anchor())<<(getrota_angle()));
    p3= getrota_anchor() + ((p3-getrota_anchor())<<(getrota_angle())); p4= getrota_anchor() + ((p4-getrota_anchor())<<(getrota_angle()));
}
