...
void Car::route(int timer,int start_time, int stop_time, int slot_number){
    static Vec point(0, 0);
    std::vector<Vec> in=generate_in_control_points(slot_number);
    std::vector<Vec> back=generate_back_control_points(slot_number);
    std::vector<Vec> out=generate_out_control_points(slot_number);
    if (0<=timer-start_time&&timer-start_time<10*(int)size(in)){
        point=generate_route(in,timer-start_time);
        rotate(getAnchor(), (float)atan2((point-getAnchor()).getY(), (point-getAnchor()).getX())-getangle());
        move(point-getAnchor());
    }
    else if (0<=timer-start_time&&timer-start_time<10*(int)size(in)+10*(int)size(back))
    {
        point=generate_route(back,timer-start_time-10*(int)size(in));
        rotate(getAnchor(),(float)atan2((point-getAnchor()).getY(), (point-getAnchor()).getX())-getangle());
        move(point-getAnchor());
        rotate(getAnchor(),PI);
        if (timer-start_time==10*(int)size(in)) rotate(getAnchor(),PI);
    }
    else if (timer-start_time<10*(int)size(in)+10*(int)size(back)&&timer-start_time-stop_time-10*(int)size(in)-10*(int)size(back)<0)
    {
        point=generate_route(back,10*(int)size(back));
        rotate(getAnchor(),(float)atan2((point-getAnchor()).getY(), (point-getAnchor()).getX())-getangle());
        move(point-getAnchor());
        rotate(getAnchor(),PI);
    }
    else if (0<=timer-start_time-stop_time-10*(int)size(in)-10*(int)size(back)&&timer-stop_time-start_time<10*(int)size(in)+10*(int)size(back)+10*(int)size(out))
    {
        point=generate_route(out,timer-start_time-stop_time-10*(int)size(in)-10*(int)size(back));
        rotate(getAnchor(), (float)atan2((point-getAnchor()).getY(), (point-getAnchor()).getX())-getangle());
        move(point-getAnchor());
        if (timer-start_time-stop_time==10*(int)size(in)+10*(int)size(back)) rotate(getAnchor(),PI);
    }
    in.clear(); back.clear(); out.clear();
}
Car::~Car(){ deletesh(); }
...
void UFO::route(int timer,int start_time, int stop_time, int slot_number){
    static Vec point(0, 0);
    std::vector<Vec> in=generate_in_control_points(slot_number);
    std::vector<Vec> back=generate_back_control_points(slot_number);
    std::vector<Vec> out=generate_out_control_points(slot_number);
    ...
    else if (0<=timer-start_time-stop_time-10*(int)size(in)-10*(int)size(back)&&timer-start_time-stop_time<10*(int)size(in)+10*(int)size(back)+10*(int)size(out))
    {
        point=generate_route(out,timer-start_time-stop_time-10*(int)size(in)-10*(int)size(back));
        rotate(getAnchor(), (float)atan2((point-getAnchor()).getY(), (point-getAnchor()).getX())-getangle());
        move(point-getAnchor());
...
void Spacecraft::route(int timer,int start_time, int stop_time, int slot_number){
    static Vec point(0, 0);
    std::vector<Vec> in=generate_in_control_points(slot_number);
    std::vector<Vec> back=generate_back_control_points(slot_number);
    std::vector<Vec> out=generate_out_control_points(slot_number);
